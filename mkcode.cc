...
int procev(RecoEvent &e0)
{
  ...
  if(ievproc == 0) { // initialization
    ...
    //new histos
    cellEnergyHist = new TH2F("cellEnergyHist", "Cell Energy Distribution", 6, 0, 6, 6, 0, 6);
    //  cellEnergyHistograms = new TH1F*[NCellsECALX];
...
//  timehisto(e0);
  ...
  double emaxcell=0.;
  TH1F** cellEnergyHistograms = new TH1F*[NCellsECALX];
  for (int x = 0; x < NCellsECALX; ++x) {
    cellEnergyHistograms[x] = new TH1F[6]; 

   for (int y = 0; y < 6; ++y) {
     std::string histName = "CellEnergyHist_" + std::to_string(x) + "_" + std::to_string(y);
     new (&cellEnergyHistograms[x][y]) TH1F(histName.c_str(), histName.c_str(), 100, 0, 100);
